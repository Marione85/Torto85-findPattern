...
class OthelloNNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default(): 
            ...
            h_conv1 = tf.keras.layers.ReLU()(tf.keras.layers.BatchNormalization()(self.conv2d(x_image, args.num_channels, 'same'), training=self.isTraining))
            ...
            h_conv2 = tf.keras.layers.ReLU()(tf.keras.layers.BatchNormalization()(self.conv2d(h_conv1, args.num_channels, 'same'), training=self.isTraining))

            h_conv3 = tf.keras.layers.ReLU()(tf.keras.layers.BatchNormalization()(self.conv2d(h_conv2, args.num_channels, 'valid'), training=self.isTraining))

            h_conv4 = tf.keras.layers.ReLU()(tf.keras.layers.BatchNormalization()(self.conv2d(h_conv3, args.num_channels, 'valid'), training=self.isTraining))

            h_conv4_flat = tf.reshape(h_conv4, [-1, args.num_channels*(self.board_x-4)*(self.board_y-4)])

            s_fc1 = Dropout(self.dropout)(Relu(BatchNormalization(axis=1)(Dense(1024, use_bias=False)(h_conv4_flat), training=self.isTraining))) # batch_size x 1024
            s_fc2 = Dropout(self.dropout)(Relu(BatchNormalization(axis=1)(Dense(512, use_bias=False)(s_fc1), training=self.isTraining)))         # batch_size x 512
    ...
    def conv2d(self, x, out_channels, padding):
      ...
      #print(padding); 
      #padding="same" 
      return tf.layers.conv2d(x, out_channels, kernel_size=[1,1], padding=padding, use_bias=False)
...
class ResNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default(): 
            ...
            x_image = tf.keras.layers.batch_normalization(x_image, axis=1, name='conv_bn', training=self.isTraining)
            ...
            policy = tf.keras.layers.batch_normalization(policy, axis=3, name='bn_pi', training=self.isTraining)
            policy = tf.nn.relu(policy)
            policy = tf.keras.layers.flatten(policy, name='p_flatten')
            self.pi = tf.keras.layers.dense(policy, self.action_size)
            self.prob = tf.nn.softmax(self.pi)

            value = tf.keras.layers.conv2d(residual_tower, 1,kernel_size=(1, 1), strides=(1, 1),name='v',padding='same',use_bias=False)
            value = tf.keras.layers.batch_normalization(value, axis=3, name='bn_v', training=self.isTraining)
            value = tf.nn.relu(value)
            value = tf.keras.layers.flatten(value, name='v_flatten')
            value = tf.keras.layers.dense(value, units=256)
            value = tf.nn.relu(value)
            value = tf.keras.layers.dense(value, 1)
    ...
    def residual_block(self,inputLayer, filters,kernel_size,stage,block):
        ...
        residual_layer = tf.keras.layers.batch_normalization(residual_layer, axis=3, name=bn_name+'2a', training=self.isTraining)
        ...
        residual_layer = tf.keras.layers.batch_normalization(residual_layer, axis=3, name=bn_name+'2b', training=self.isTraining)
