...
class Board():
    ...
    def __init__(self, n):
        ...
        # in posizione (i,j,0) abbiamo il numero di bianchi ottenuti come
        # risposte interrogando la casella (i,j)
        # in posizione (i,j,1) abbiamo il numero di neri ottenuti come
        # risposte interrogando la casella (i,j)
        ...
        # -1 non pattern, 1 pattern
        self.case = self.existPattern()

    # aggiunge la sintassi [][] alla Board
    ...
    def get_legal_moves(self, player):
        ...
        if player == 1:
            ...
            return list(itertools.product(np.argwhere(self.mask2 == 1).tolist(),
                np.argwhere(self.mask1 == 1).tolist())) + ["guess_pattern",
                "guess_nonpattern"]
        else:
            # il primo elemento della coppia risultante è quello da scoprire
            # il secondo elemento della coppia risultante è quello da interrogare
            return list(itertools.product(np.argwhere(self.mask1 == 1).tolist(),
                np.argwhere(self.mask2 == 1).tolist())) + ["guess_pattern",
                "guess_nonpattern"]

    def execute_move(self, move, player):
        """Esegue le due mosse sulla "board"; in realtà modifica le maschere e
        le informazioni, a seconda del player. (1 per player1, -1 per player2)"""
